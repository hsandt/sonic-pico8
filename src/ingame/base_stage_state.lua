...
require("data/picomap_data")
require("platformer/picomap_runtime")
...
-- Runtime PicoMap loading code based on picomap v0.42 testcart by jadelombax
-- https://www.lexaloffle.com/bbs/?tid=42848&pid=91798

function base_stage_state:picomap_init()
 fg_array={}
 lvlnum=1
 bp=32
 move_data()
...
function base_stage_state:picomap_update()
 if bp>15 then
 ...
 camx=mid(0,camx+(int_div(bn,2)%2-bn%2)*2,fgw*8-128)
 camy=mid(0,camy+(int_div(bn,8)%2-int_div(bn,4)%2)*2,fgh*8-screen_ht*8)
 lvlnum=mid(1,lvlnum+int_div(bp,32)%2-int_div(bp,16)%2,#loc_tbl-1)

 cls(bgc)
 map_array(fg_array,1,0,0)
end

-- COMMENTED OUT as replaced by PicoMap
