...
describe('codetuner', function ()
  ...
  describe('tuned_variable', function ()
    ...
    describe('__eq', function ()
      it('should return true if tile vectors have the same name and default value', function ()
        local tuned_var1 = codetuner.tuned_variable("offset", 6)
        local tuned_var2 = codetuner.tuned_variable("offset", 6)
        assert.is_true(tuned_var2:__eq(tuned_var1))
      end)
      it('should return false if tile vectors have different names', function ()
        local tuned_var1 = codetuner.tuned_variable("offset", 6)
        local tuned_var2 = codetuner.tuned_variable("offset2", 6)
        assert.is_false(tuned_var2:__eq(tuned_var1))
      end)
      it('should return false if tile vectors have different default values', function ()
        local tuned_var1 = codetuner.tuned_variable("offset", 6)
        local tuned_var2 = codetuner.tuned_variable("offset", 12)
        assert.is_false(tuned_var2:__eq(tuned_var1))
      end)
    end)
  ...
  describe('codetuner:update_window', function ()
    ...
    it('should call gui:update', function ()
      ...
      assert.spy(update_stub).was_called_with(codetuner.gui)
    end)
  ...
  describe('codetuner:render_window', function ()
    ...
    it('should call gui:draw', function ()
      ...
      assert.spy(draw_stub).was_called_with(codetuner.gui)
    end)
